...
sw_multiplier_list = np.linspace(0, 50, 3)
...
def heldoutLaser_sw(session, omitCtrlBounds, psth_newbounds, og_psth_bounds, og_psth_ctrl_bounds, weight, sw_multiplier):
    ...
    for held_out_num in num_lasers:
        ...
        sw_binned = (np.hstack(sw_list))[:-1]
        sw_boolean = np.full(sw_binned.size, True)
        sw_boolean[sw_binned < 1] = False

        sw = np.ones(sw_boolean.size)
        sw[sw_boolean] = sw_multiplier
        print('formatting!')
...
pdump(output, '../picklejar/LaserSWSweep/newreg_output.pickle')
