...
def analyze_session(reg, session, num_PCs, post, nlag, binsize, sigma):
    try:
        ...
        return (h, l_rsquare, c_rsquare), (reg, session.name, session.inactivate)
...
pdump(session_output,
        f'../../picklejar/coSweepRegsLaser_{nlag}nlag_{post}post.pickle')
pdump(session_asymps,
        f'../../picklejar/coSweepRegsLaserasymps_{nlag}nlag_{post}post.pickle')
