...
script_name = 'coSweepRegsCtrlLinear'
num_workers = 30
session_path_list = []
session_name_list = []
co_path = '../../data/co'
...
def analyze_session_asymps(session, num_PCs, post, nlag, binsize, sigma):
    try:
        ...
        return (r1_rsquare, h_r1), (r2_rsquare, h_r2)
...
session_output = []
session_asymps = []
for session_path in session_path_list[::3]:
    try:
        session = MPOptoClass(session_path)
        reg_terms = np.linspace(0,10000,20)
...
pdump(session_output,
        f'../../picklejar/coSweepRegsCtrlLinear_{nlag}nlag_{post}post_10sw.pickle')
pdump(session_asymps,
        f'../../picklejar/coSweepRegsCtrlLinearasymps_{nlag}nlag_{post}post_10sw.pickle')
