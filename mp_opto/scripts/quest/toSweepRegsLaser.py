...
def analyze_session_asymps(session, num_PCs, post, nlag, binsize, sigma):
    try:
        ...
        (X,Y), PCs = session.generate_trainset(train_lasers, binsize=binsize,
        ...
        return r1_rsquare, r2_rsquare, (X_l, X_c), (Y_l, Y_c)
...
def analyze_session(reg, session, num_PCs, post, nlag, binsize, sigma):
    try:
        ...
        return (h, l_rsquare, c_rsquare), (X_l, X_c), (Y_l, Y_c), (reg, session.name, session.inactivate)
...
pdump(session_output,
        f'../../picklejar/toSweepRegsLaser_{nlag}nlag_{post}post.pickle')
pdump(session_asymps,
        f'../../picklejar/toSweepRegsLaserasymps_{nlag}nlag_{post}post.pickle')
